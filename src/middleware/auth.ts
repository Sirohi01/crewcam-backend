...
export const authenticate = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Prioritize the Authorization header explicitly provided by the client, then fall back to cookies.
    const token = req.headers.authorization?.split(' ')[1] || req.cookies.token_employer || req.cookies.token_super_admin;
